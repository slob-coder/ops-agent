...
class NotebookProtocol(Protocol):
    """Notebook 接口协议 — Basic 和 Smart 实现都必须满足。

    smart-notebook 作为可选插件，无需 import 此 Protocol，
    只需保证 duck-typing 兼容即可。此 Protocol 主要用于
    ops-agent 内部的类型注解和 isinstance 检查。
    """
    ...
    def append_to_incident(self, filename: str, content: str) -> None: ...
    def close_incident(self, filename: str, summary: str) -> None: ...
    def read_incident(self, filename: str) -> str: ...

    # ── 对话 ──
    def log_conversation(self, role: str, message: str) -> None: ...
    def get_recent_conversation(self, limit: int = 20) -> str: ...

    # ── 成长 / Git ──
    def update_readme_growth(self) -> None: ...
