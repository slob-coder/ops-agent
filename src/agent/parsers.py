...
class ParsersMixin:
    ...
    def _parse_watchlist_commands(self, watchlist_text: str, patrol_round: int = 0) -> list[str]:
        ...
        通用方案：按 ### 标题自动识别层级，顺序即优先级。
        - 第 1 个 ### 段落 = 最高优先级，每轮必检 (every_n=1)
        - 第 2 个 = every_n=2，第 3 个 = every_n=4，第 4 个 = every_n=8 ...
        - every_n = 2^(tier_index)，保证 2^N 轮内全部覆盖

        不依赖任何特定命名、emoji 或格式约定，只要 watchlist 用 ### 分段即可。
        """
        # 1. 按 ### 标题切分段落
        tiers: list[tuple[str, list[str]]] = []  # [(section_title, [cmd, ...])]
        ...
        # 2. 按轮转策略筛选：tier_index → every_n = 2^index
        result = []
        for tier_index, (_, cmds) in enumerate(tiers):
            every_n = 2 ** tier_index
        ...
        logger.debug(
            ...
            f"tiers={len(tiers)}, "
            f"tier_sizes={[len(c) for _, c in tiers]}, "
