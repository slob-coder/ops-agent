...
## 相关代码搜索结果（当源码定位失败时的 fallback）
以下是 Agent 通过关键词搜索定位到的相关源码片段。**如果上一节为空但这一节不为空，说明已找到需修改的代码位置，steps 应直接给出修复操作，不要再要求查看文件。**

{code_search_results}

## 已确认事实（无需再验证）
基于诊断阶段已确认的事实，**不要再在 gaps 中要求查看这些信息**。如果你发现自己在 steps 或 gaps 中写了查看命令来确认这些事实，请删除。
